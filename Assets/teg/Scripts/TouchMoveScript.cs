...
public class TouchMoveScript : MonoBehaviour {
	...
	void Update () 
	{
		...
		if(Input.GetMouseButton(0))
		{
			...
			{
				player1Pos = new Vector3(-7.5f, 0.5f, touchCache.y);//Mathf.Clamp(mouseCache.y / screenHeight * speed, 0, 8)
			}
		...
		if (Input.touchCount >= 1)
		{
			...
			foreach (Touch touch in Input.touches)
			{
				...
				player1Pos = new Vector3(-7.5f, 0.5f,touchCache.y );//Mathf.Clamp(touchCache.y / screenHeight * 8, 0, 8)
